...
router.patch('/:userId/follow', isLoggedIn, async(req, res, next) => { // PATCH /user/1/follow
  try {
    ...
    if (!user) {
      res.status(403).send('없는 사람을 팔로우하려고 하시나요?');
    }
...
router.delete('/:userId/unfollow', isLoggedIn, async(req, res, next) => { // DELETE /user/1/unfollow
  try {
    const user = await User.findOne({ where: { id: req.params.userId }});
    if (!user) {
      res.status(403).send('없는 사람을 언팔로우하려고 하시나요?');
    }
...
router.get('/followers', isLoggedIn, async(req, res, next) => { // GET /user/followers
  try {
    ...
    if (!user) {
      res.status(403).send('없는 사람을 찾으려고 하시나요?');
    }
...
router.get('/followings', isLoggedIn, async(req, res, next) => { // GET /user/followings
  try {
    ...
    const folloings = await user.getFollowings(req.user.id);
...
router.delete('/follower/:userId', isLoggedIn, async(req, res, next) => { // DELETE /user/follower/1
  try {
    const user = await User.findOne({ where: { id: req.user.id }});
    if (!user) {
      res.status(403).send('없는 사람을 차단하려고 하시나요?');
    }
